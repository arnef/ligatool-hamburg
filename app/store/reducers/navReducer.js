...
export default (nav: NavState = defaultState, action: Action) => {
  switch (action.type) {
    ...
    case NavigationActions.NAVIGATE: {
      ...
      const newAction = idx > -1
        ? { ...action, routeName: `${tab}${tabRoutes[idx]}` }
        : { ...action };
    ...
    case NavigationActions.BACK: {
      if (Platform.OS === ANDROID) {
        ...
        if (!nav.drawerOpen && !nav.modalOpen) {
          if (
            nav.actionStacks[nav.currentTab] &&
            nav.actionStacks[nav.currentTab].length > 0
          ) {
            ...
            nav.state = Navigator.router.getStateForAction(
              NavigationActions.navigate(prevTab),
              nav.state
            );
          }
        } else {
