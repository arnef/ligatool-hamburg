...
class PlayerView extends Component {
  ...
  render() {
    const player =
      this.state.player || this.props.navigation.state.params.player;
    const props = {
    ...
    return (
      <Container {...props}>
        <ListItem.Group>
          ...
          {this.renderRow(
            'Organisation',
            player.association ? player.association.name : null
          )}
          {player.teams &&
          ...
          {player.statistics &&
            ...
            <View style={{ paddingHorizontal: 12, paddingVertical: 3 }}>
              ...
              <Row fluid style={{ justifyContent: 'space-around' }}>
                <Column center>
                  <Text bold>Pos.</Text>
                  <Text>{player.statistics[0].position}</Text>
                </Column>
                <Column center>
                  <Text bold>Quote</Text>
                  <Text>{player.statistics[0].rate}</Text>
                </Column>
                <Column center style={{ marginLeft: 10 }}>
                  <Text bold>Spiele</Text>
                  <Text>{player.statistics[0].matches}</Text>
                </Column>
                <Column center>
          ...
          {player.ranglists &&
            player.ranglists.length > 0 &&
            <View style={{ paddingHorizontal: 12, paddingVertical: 3 }}>
              <Text bold>
                Ranglistenplatzierungen {player.ranglists[0].season}
              </Text>
              {player.ranglists.map((item, idx) => {
                return (
                  <Row
                    fluid
                    key={item.id}
                    style={idx % 2 === 1 ? { backgroundColor: '#eee' } : {}}
                  >
                    <Text>{item.name}</Text>
                    <Column />
          ...
          {player.tournament_participations &&
            player.tournament_participations.length > 0 &&
            <Row>
              <Column>
...
PlayerView.navigationOptions = {
  title: ({ state }) =>
    `${state.params.player.name} ${state.params.player.surname}`
};
