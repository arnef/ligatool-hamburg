...
import { Text, TeamLogo, ListItem, Separator, Card, Image } from '../../../../../components';
...
export class NoSets extends React.PureComponent<Props> {
  ...
  private onOpenVenue = () => {
    ...
    const address = encodeURI(`${venue.street}, ${venue.zipCode} ${venue.city}`)
  ...
  private onDateChange = () => {
    //TODO: this.props.navigate(Routes..., { id: this.props.match.id });
    alert('no route yey')
  }
    ...
    return (
      <Card onPress={this.onOpenPlayer(player)}>
        <View style={styles.playerContainer}>
          ...
          <Text style={styles.playerText}>{`${player.name} ${player.surname}`}</Text>
        </View>
      </Card>
    )
  }
  ...
  private renderPlayer = () => {
    ...
    const length = player.home ? Math.max(player.home.length, player.away.length) : 0;
  ...
  public render() {
    ...
    return (
      <View style={styles.container}>
            ...
            <View style={styles.firstMatch}>
              ...
              <View style={styles.firstMatchResult}>
                <Text style={styles.textFirstMatchResult}>
                  { `${firstFixture.setPointsAwayTeam}:${firstFixture.setPointsHomeTeam}`}
                </Text>
                <Text style={styles.textFirstMatchResult}>
                  {`(${firstFixture.goalsAwayTeam}:${firstFixture.goalsHomeTeam}`}
                </Text>
              </View>
              <Text small secondary bold>{' '}</Text>
            </View>
          )}
          <View style={styles.teamInfo}>
        ...
        <ListItem.Group>
          <ListItem.Header title={`${match.status === 'POSTPONED' ? Strings.SO_FAR : ''}`} />
          { venue && !!venue.name && (
            <ListItem onPress={this.onOpenVenue}>
              <View style={styles.option}>
                <Text>{`${venue.name}`}</Text>
                <Text>{`${venue.street}, ${venue.zipCode} ${venue.city}`}</Text>
              </View>
              <ListItem.Icon name="pin" color={color} right />
            </ListItem>
          )}
          { isAdmin && <Separator />}
          { isAdmin && ASSOC.indexOf('tfvhh') !== -1 && (
            <ListItem 
              onPress={this.onDateChange}
            >
              <Text style={styles.option}>{ Strings.CHANGE_MATCH_DATETIME}</Text>
