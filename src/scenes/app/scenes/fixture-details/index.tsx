...
import { MatchHeader, Content, Switch, SetItem, Button } from '../../../../components';
import { Strings } from '../../../../lib/strings';
import { getFixture, getFixtureModus, getFirstFixture, getFixturePlayerHomeList, getFixturePlayerAwayList, getFixtureVenue, setFixtureModus, suggestFixtureResult, acceptFixtureResult, setFixtureStatusInPlay } from 'src/redux/modules/fixtures';
import { getNavigationStateParams, showPlayer, navigate } from 'src/redux/modules/navigation';
      ...
      <View style={styles.container}>
        ...
        { isAdmin && showButton && (
          <Button 
            ...
            square
            title={Strings.SCORE_BUTTON_TEXT[
              match.status === 'IN_PLAY' ? 0 : (
                actionRequired ? 2 : 1
              )
            ]}
...
function mapStateToProps(state: any, props: Props): StateProps {
  ...
  const isAdmin = (accessForTeams(state).indexOf(fixture.homeTeamId) !== -1 || accessForTeams(state).indexOf(fixture.awayTeamId) !== -1)
  ...
  const getShowButton = () => {
    ...
    if (fixtureModus.fixture < 0) {
      return fixture.result && fixture.result.setPointsHomeTeam + fixture.result.setPointsAwayTeam === Math.abs(fixtureModus.fixutre);
    }

    return fixture.result && (fixture.result.setPointsHomeTeam >= fixtureModus.fixture || fixture.result.setPointsAwayTeam >= fixtureModus.fixture);
  }
  return {
    match: fixture,
    isAdmin: isAdmin,
    ...
    actionRequired: accessForTeams(state).indexOf(fixture.suggestingTeamId) === -1,
...
function mapDispatchToProps(dispatch: Dispatch<any>, props: Props): DispatchProps {
  const fixtureId = getNavigationStateParams(props.navigation).id;
  return {
    selectPlayer: (data: any) => dispatch(showPlayer(fixtureId, data)),
    openTeam: (id: string, name: string) => dispatch(navigate({
      routeName: Routes.teamDetails,
      params: { team: { id }, title: name },
    })),
    getMatch: () => dispatch(getMatch(fixtureId)),
    setType: (modus: any, resetGameNumbers: any) => dispatch(setFixtureModus(fixtureId, modus, resetGameNumbers)),
    ...
    navigate: (routeName: string, params: any) => dispatch(navigate({ routeName, params })),
  };
}

export const FixtureDetails = connect(mapStateToProps, mapDispatchToProps)(Fixture);