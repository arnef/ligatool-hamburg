...
import { NavigationContainer, StackNavigator, TabNavigator, NavigationRouteConfigMap } from "react-navigation";
import { Routes } from "src/scenes/routes";
import { TabBarIcon, TabBarBottom, headerNavigationOptions } from "src/containers/navigation";
...
function createTabStackNavigator(key: string, screen: NavigationContainer | React.ComponentClass, title: string, screens?: any) {
const defaultScreens = {
  ...
  [Routes.fixtureDetails]: {
    ...
    navigationOptions: ({ navigation }: any) => ({
      title: getNavigationStateParams(navigation) && getNavigationStateParams(navigation).title
...
return StackNavigator(screens ? { ...defaultScreens, ...screens } : defaultScreens, headerNavigationOptions);
}
  ...
  for (let tab of tabs) {
    tabsMap[tab.route] = {
      screen: tab.customStack 
        ? tab.screen 
        : createTabStackNavigator(tab.route, tab.screen, tab.title, tab.screens),
      navigationOptions: {
        tabBarLabel: tab.title,
        tabBarIcon: ({ tintColor }: { tintColor: string }) => <TabBarIcon name={tab.icon} color={tintColor} />,
      }
    }
  }
  return tabsMap;
...
export const AppScenes = TabNavigator(createTabs([
  { route: Routes.overview, screen: Overview, title: Strings.OVERVIEW, icon: 'football' },
  { route: Routes.myTeam, screen: MyTeam, title: Strings.MY_TEAM, icon: 'shirt' },
  { 
  ...
  { route: Routes.settings, screen: Settings, title: Strings.SETTINGS, icon: 'settings', customStack: true }
]), {
